#!/usr/bin/env python3
"""Generate remaining test images on CPU for stability."""
...
def main():
    """Generate remaining 2 test images on CPU."""
    
    output_dir = Path("tests/data/images")
    
    print("Loading Stable Diffusion 1.5 pipeline...")
    
    # Load the pipeline - force CPU for stability
    ...
    for i, case in enumerate(remaining_cases, 3):
        ...
        try:
            # Generate image with minimal steps for speed
            image = pipe(
                case["prompt"],
                num_inference_steps=15,  # Fewer steps for CPU
                guidance_scale=7.5,
                height=512,
                width=512
            ).images[0]
